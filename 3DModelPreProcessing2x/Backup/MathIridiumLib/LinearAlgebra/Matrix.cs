...
using System;
using System.Text;
using Iridium.Numerics.Properties;
using Iridium.Numerics.Distributions;
using System.Collections.Generic;

namespace Iridium.Numerics.LinearAlgebra
{
        ...
        ICloneable
    {
        ...
        /// <summary>
        /// Construct an m-by-n matrix of zeros (m rows, n columns).
        /// </summary>
        ...
        /// <summary>
        /// Copies a specified column of this matrix to a new matrix.
        /// </summary>
        public
        Matrix
        GetColumnMatrix(
        ...
        {
            if (columnIndex < 0 || columnIndex+no-1 >= _columnCount || no == 0)
            {
                throw new ArgumentOutOfRangeException("columnIndex or no");
            }

            Matrix newData = new Matrix(_rowCount, no);

            for (int j = columnIndex; j < columnIndex + no; j++)
            {
                for (int i = 0; i < _rowCount; i++)
                {
                    newData[i, j - columnIndex] = _data[i][j];
                }
            }
            return newData;
        }
        ...
        ToString()
        {
            ...
            for(int i = 0; i < _rowCount; i++)
            {
                ...
                for(int j = 0; j < _columnCount; j++)
                {
                    ...
                    sb.Append(_data[i][j].ToString(System.Globalization.CultureInfo.InvariantCulture));
                }
